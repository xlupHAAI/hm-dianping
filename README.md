...
2026.01.19	ver1.3.3 基于 Redis 实现了一个简单的分布式锁

- 利用 `setnx`的原子性质加锁，解锁使用 lua 脚本保证“判断是否当前线程持有”和“解锁”两个操作的原子性。后续还是使用 Redission，支持锁重入、锁重试和自动续期等。

2026.01.20	ver1.3.4 使用 Redission 代替了基于 MySQL 的分布式锁
